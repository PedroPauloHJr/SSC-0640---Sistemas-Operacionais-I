...
void printPageTable(vector<Page *> table){//Printa a tabela de páginas de um processo
    ...
    {
        cout << "Modificada: " << table[i]->getM() << "|Presente: " <<table[i]->getP()<< "|Referenciada: " << table[i]->getR()<< "|Indice Quadro: " << table[i]->getQuadro() << "|contador: " <<table[i]->counter <<"\n";
    }
    cout << "***********************FIM ************************************\n";
...
void swapNode(No *n){//Substitui um no por outro
    ...
    cout << "PAGINA REMOVIDA COM O CONTADOR = " << less->getPagina()->counter << "\n";
    if(less->getPagina()->getM()){
        cout << "A PAGINA ESTAVA MODIFICADA E SUAS ALTERACOES FOREM SALVAS";
        less->getPagina()->setM(false);
    }
    less->getPagina()->setP(false);
    if(less == mainMemory){
        n->getPagina()->setQuadro(mainMemory->getPagina()->getQuadro());
        n->setProx(mainMemory->getProx());
        mainMemory = n;
    ...
    while(aux2->getProx() != NULL){
        if(aux2->getProx() == less){
            n->setProx(less->getProx());
            n->getPagina()->setQuadro(less->getPagina()->getQuadro());
            aux2->setProx(n);
            return;
        }
        aux2 = aux2->getProx();
    ...
    if(a == 'C'){
        ...
        *pro = processes;
        ...
        if(logicToFisic(op,processAux.table) == "PAGE NOT FOUND ERRO"){
            ...
            novo->getPagina()->counter = counter;
            ...
            processAux.table[getIndex(op)]->counter = counter;
